# https://geonetwork-opensource.org/manuals/3.12.x/en/install-guide/installing-from-source-code.html
ARG MAVEN_IMAGE_TAG=3.8-eclipse-temurin-8
ARG TOMCAT_IMAGE_TAG=9-jre8-temurin-jammy
...
RUN git checkout tags/${GEONETWORK_VERSION} -b docker-builder && \
    git submodule init && \
...
FROM tomcat:${TOMCAT_IMAGE_TAG} AS RELEASE
...
LABEL org.opencontainers.image.source https://github.com/RogerioBorba/geonetwork-docker
LABEL org.opencontainers.image.authors "Carlos Eduardo Mota <carlos.mota@sgb.gov.br>; Rogerio Borba <rogerio.borba@ibge.gov.br>"
